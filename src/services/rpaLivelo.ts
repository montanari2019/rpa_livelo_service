...
import { ExtratTransactionType, LiveloCredentialsType, ProcessQueueRoutineMessages } from './@type';
import { finishPuppeteer, goToPuppeter, initPuppeteer } from '../helpers/puppteer';
import { decryptPassword } from '../helpers/crypto';

/**
 * Passo 1: acessar página inicial e limpar cookies/localStorage
 */
async function accessingHomePage(page: Page, logs: ProcessQueueRoutineMessages, orderCounter: number): Promise<number> {
    try {
        logs.push({
            order: orderCounter++,
        ...
        await goToPuppeter(page, 'https://www.livelo.com.br/');
...
> {
    ...
    try {
        ...
        const anyBalancePoints = await elementBalancePoints.evaluate((el: any) => el.textContent?.trim() ?? '');
 ...
 */
export async function getTotalPagesTransactions(
    page: Page,
    logs: ProcessQueueRoutineMessages,
    orderCounter: number,
): Promise<{
    ...
    try {
        ...
        do {
            ...
            for (let i = (pageIndex === 1 ? 1 : 0); i < transactionRows.length; i++) {
                ...
                let transaction: ExtratTransactionType = {
                    data: '',
                    operacao: '',
                    parceiros: '',
                    pontos: 0,
 ...
 */
export async function runLiveloRpa(credentials: LiveloCredentialsType, startOrder: number) {
    ...
    try {
        ...
        const {
            newOrderCounter: navigateOrderCounter,
            success: navigateResult,
        } = await navigateToExtract(page, logs, orderCounter);
        ...
        await finishPuppeteer(browser, page);
