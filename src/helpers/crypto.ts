...
const SALT_LENGTH = cryptoConfig.saltLength;

// Gerar uma chave a partir da chave mestre do ambiente
function getEncryptionKey(): Buffer {
    const masterKey = cryptoConfig.masterKey;
...
export function encryptPassword(plaintext: string): string {
    try {
        ...
        const derivedKey = crypto.pbkdf2Sync(key, salt, 10000, KEY_LENGTH, 'sha512');
...
export function decryptPassword(encryptedHex: string): string {
    try {
        ...
        const derivedKey = crypto.pbkdf2Sync(masterKey, salt, 10000, KEY_LENGTH, 'sha512');
